...
#define BATTERY_DESC	"Pil"
#define AC_ADAPTER_DESC "Adaptör"
#define THERMAL_DESC	"Termal"
#define COOLING_DESC	"Soğutucu"
...
static struct field *parse_field(char *buf, char *given_attr)
{
    ...
    if (!rval || !attr || !value) {
	fprintf(stderr, "Bellek bitti. parse_field alanında bellek ayrılamadı.\n");
	exit(1);
    }
...
static struct list *get_info(char *device_name, int proc_interface)
{
    ...
    if (filename == NULL) {
	fprintf(stderr, "Bellek bitti. get_info alanında bellek ayrılamadı.\n");
	return NULL;
    }
			  ...
			  int proc_interface)
{
	...
	fprintf(stderr, "Çekirdekte ACPI desteği yok, yada yanlış acpi_path (\"%s\").\n", acpi_path);
	exit(1);
    }
    ...
    if (!found_data) {
	fprintf(stderr, "Aygıt türü için destek yok: %s\n", device_type);
	return NULL;
    }
...
void print_battery_information(struct list *batteries, int show_empty_slots, int show_capacity)
{
	...
	if (type_battery) {	/* or else this is the ac_adapter */
		...
		printf("%s %d: %s, %d%%", BATTERY_DESC, battery_num - 1, statestr, percentage);

		if (present_rate == -1) {
		    poststr = "oran bilgisi yok";
		    seconds = -1;
			...
			poststr = " sürede dolacak";
		    } else {
			poststr = "sıfır oranda şarj oluyor - tam şarj olmayacak.";
			seconds = -1;
		    }
		} else if (!strcasecmp(state, "discharging")) {
		    if (present_rate > MIN_PRESENT_RATE) {
			seconds = 3600 * remaining_capacity / present_rate;
			poststr = " sürede bitecek";
		    } else {
			poststr = "sıfır oranda boşalıyor - tam boşalmayacak.";
			seconds = -1;
		    }
		} else {
		    poststr = NULL;
		...
		if (show_capacity && design_capacity > 0) {
		    ...
		    printf ("%s %d: Tasarlanan kapasite %d %s, Son tam kapasite %d %s = %d%%\n",
...
void print_ac_adapter_information(struct list *ac_adapters, int show_empty_slots)
{
	...
	while (fields) {
		...
		state = get_unit_value(value->value) ? "takılı" : "çıkmış";
	    else if (!strcmp(value->attr, "type"))
	...
	if (type_zone) {	/* or else this is a cooling device */
		...
		if (show_trip_points) {
			...
			if (trip[i].trip_temp >= MIN_TEMP) {
				...
				printf("%s %d: %d noktası %s durumuna geçilecek sıcaklık %.1f %s\n",
...
void print_cooling_information(struct list *cooling, int show_empty_slots)
{
	...
	if (type_cooling) {	/* or else this is a thermal zone */
		...
		printf("%s %d: %s durum bilgisi alınamadı\n", COOLING_DESC, sensor_num - 1, type);
	    } else {
